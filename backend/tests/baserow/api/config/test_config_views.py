...
def test_get_config(api_client):
    response = api_client.get(reverse('api:config:get'))
    assert response.status_code == HTTP_200_OK
    response_json = response.json()
    assert response_json['allow_new_signups'] is True

    config = Config.objects.first()
    config.allow_new_signups = False
    config.save()
...
@pytest.mark.django_db
def test_update_config(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token(is_staff=True)
    user_2, token_2 = data_fixture.create_user_and_token()

    response = api_client.patch(
        reverse('api:config:update'),
        {'allow_new_signups': False},
        format='json',
        HTTP_AUTHORIZATION=f'JWT {token_2}'
    )
    assert response.status_code == HTTP_403_FORBIDDEN
    assert CoreHandler().get_config().allow_new_signups is True
    ...
    response_json = response.json()
    assert response_json['allow_new_signups'] is False
    assert CoreHandler().get_config().allow_new_signups is False
