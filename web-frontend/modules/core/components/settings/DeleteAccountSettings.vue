<template>
  <div>
    <h2 class="modal__title">{{ $t('deleteAccountSettings.title') }}</h2>
    ...
    <div v-if="!success" class="modal__footer">
      <Button type="danger" :loading="loading" @click="deleteAccount">
        {{ $t('deleteAccountSettings.submitButton') }}</Button
      >
    </div>
  </div>
</template>
