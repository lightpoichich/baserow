...
import { makeRefreshAuthInterceptor } from "~/modules/core/plugins/clientAuthRefresh";
...
const createAxiosInstance = (app) => {
  ...
  const url =
    (process.client
      ...
      : runtimeConfig.privateBackendUrl) + "/api";
...
export default defineNuxtPlugin({
  async setup(nuxtApp) {
    ...
    const client = createAxiosInstance(nuxtApp);
    ...
    client.interceptors.response.use(
      null,
      makeErrorResponseInterceptor(store, nuxtApp, clientErrorMap, error)
    );
