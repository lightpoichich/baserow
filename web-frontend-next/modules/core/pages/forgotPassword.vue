<template>
  <div>
    ...
    <div v-if="success" class="box__message">
      ...
      <NuxtLink :to="{ name: 'login' }" class="button button--large">
        <i class="fas fa-arrow-left"></i>
        {{ $t("action.backToLogin") }}
      </NuxtLink>
    </div>
...
export default {
  components: { LangPicker, Error, Alert },
  mixins: [error],
  setup() {
    const { t } = useI18n();
    useHead({
  ...
  methods: {
    async sendLink() {
      ...
      try {
        ...
        const resetUrl = `${runtimeConfig.public.publicWebFrontendUrl}/reset-password`;
        await AuthService(nuxtApp.$client).sendResetPasswordEmail(
          this.account.email,
          resetUrl
        );
        this.success = true;
