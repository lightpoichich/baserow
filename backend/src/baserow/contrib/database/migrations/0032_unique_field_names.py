...
def forward(apps, schema_editor):
    ...
    with connection.schema_editor() as tables_schema_editor:
        # We need to stop the transaction because we might need to lock a lot of
        # tables which could result in an out of memory exception.
        ...
        for table_id, reserved_name in qs:
            ...
            for field in fields_with_reserved_names:
                new_name, i = find_next_unused_field_name(
                    Field, table_id, reserved_name, i
                )
                if not real_run:
                    print(
                        f"Table {table_id}: {field.name} ({field.id}) -> " f"{new_name}"
                    )
                else:
        ...
        for table_id, duplicate_name in qs:
            print(f"Working on {table_id} and {duplicate_name}")
            i = 2
            fields_with_reserved_names = Field.objects.filter(
        ...
        assert (
            not Table.objects.values("id", "field__name")
            .annotate(name_count=Count("field__name"))
            .filter(name_count__gt=1)
            .order_by()
            .exists()
        ), "There are still duplicate fields in the database, something went wrong."
        exists = Field.objects.values("id", "name").filter(name__in=["order", "id", ""])
        assert not exists.exists(), (
            "There are still fields called 'order', 'id' or '' in the db, "
