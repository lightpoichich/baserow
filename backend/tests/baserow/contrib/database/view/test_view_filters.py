...
@pytest.mark.django_db
def test_length_is_greater_than_filter_type(data_fixture):
    user = data_fixture.create_user()
    table = data_fixture.create_database_table(user=user)
    grid_view = data_fixture.create_grid_view(table=table)
    text_field = data_fixture.create_text_field(table=table)
    ...
    row_2 = model.objects.create(
        **{
            ...
            f"field_{long_text_field.id}": "tes",
            f"field_{url_field.id}": "tes@test.com",
            f"field_{email_field.id}": "tes@baserow.com",
            f"field_{phone_number_field.id}": "+44 12",
        }
    )
    row_3 = model.objects.create(
        **{
            f"field_{text_field.id}": "test",
            f"field_{long_text_field.id}": "test",
            f"field_{url_field.id}": "test@test.com",
            f"field_{email_field.id}": "test@baserow.com",
            f"field_{phone_number_field.id}": "+44 123",
        }
    )

    view_filter = data_fixture.create_view_filter(
        view=grid_view, field=text_field, type="length_is_greater_than", value=0
    )
