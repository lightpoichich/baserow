...
def test_dispatch_context_page_from_context(mock_get_field_names, data_fixture):
    ...
    dispatch_context = BuilderDispatchContext(
        request, page, offset=0, count=5, only_dispatch_public_formula_fields=True
    )
...
@pytest.mark.parametrize(
    "feature_flag_is_set,only_dispatch_public_formula_fields",
    (
        [False, True],
        [True, True],
        [False, False],
...
def test_builder_dispatch_context_field_names_computed_on_feature_flag(
    ...
    dispatch_context = BuilderDispatchContext(
        mock_request,
        mock_page,
        only_dispatch_public_formula_fields=only_dispatch_public_formula_fields,
    )

    if feature_flag_is_set and only_dispatch_public_formula_fields:
