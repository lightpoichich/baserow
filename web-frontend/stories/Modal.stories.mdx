...
export const Template = (args, { argTypes }) => ({
    ...
    template: `<div><Modal ref="modal" v-bind="$props"><template #topbar-content>${args.topbarContent}</template><template #topbar-actions>${args.topbarActions}</template>${args.content}<template #sidebar>${args.sidebar}</template><template #actions>${args.actions}</template></Modal><button ref="button" @click="openModal">open modal</button></div>`,
})
...
<Canvas>
    ...
    <Story
        name="Right sidebar and actions"
        args={{
            rightSidebar: true,
            actions:
    ...
    <Story
        ...
        args={{
            topbar: true,
            topbarContent: '<h3>Any content</h3>',
            topbarActions:
                '<li class="modal__box-topbar-icon"><i class="iconoir-bell"></i></li><li class="modal__box-topbar-icon"><i class="iconoir-message"></i></li><li class="modal__box-topbar-icon"><i class="baserow-icon-more-vertical"></i></li>',
            content:
                '<div class="modal__content">This is a simple modal content</div>',
            rightSidebar: true,
        }}
    >
        {Template.bind({})}
