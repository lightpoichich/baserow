...
from baserow.contrib.database.views.view_filters import DateParserMixin
...
@pytest.mark.django_db
def test_date_on_before_filter_type(data_fixture):
    user = data_fixture.create_user()
    table = data_fixture.create_database_table(user=user)
    grid_view = data_fixture.create_grid_view(table=table)
    date_field = data_fixture.create_date_field(table=table)
    ...
    filter = data_fixture.create_view_filter(
        view=grid_view, field=date_field, type="date_on_before", value="2021-07-06"
    )
    ids = [r.id for r in handler.apply_filters(grid_view, model.objects.all()).all()]
    assert len(ids) == 2
    assert row.id in ids
...
@pytest.mark.django_db
def test_date_on_after_filter_type(data_fixture):
    user = data_fixture.create_user()
    table = data_fixture.create_database_table(user=user)
    grid_view = data_fixture.create_grid_view(table=table)
    date_field = data_fixture.create_date_field(table=table)
    ...
    filter = data_fixture.create_view_filter(
        view=grid_view, field=date_field, type="date_on_after", value="2021-07-06"
    )
    ids = [r.id for r in handler.apply_filters(grid_view, model.objects.all()).all()]
    assert len(ids) == 2
    assert row_4.id in ids
