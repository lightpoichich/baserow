...
class BaserowBackupRunner:
    ...
    def __init__(
        ...
        jobs: Optional[int] = 1,
        additional_pg_tool_args: Optional[List[str]] = None,
    ):
        """
        ...
        :param additional_pg_tool_args: Any additional command line arguments to be
            passed to each batches call of pg_dump or pg_restore.
        """
        ...
        self.port = port
        self.jobs = jobs
        self.additional_pg_tool_args = additional_pg_tool_args or []
    ...
    def backup_baserow(
        ...
        try:
            self._open_files_and_run_backup(backup_file_name, batch_size)
    ...
    def restore_baserow(self, backup_file_name: str):
        """
        ...
        with tempfile.TemporaryDirectory() as temporary_directory_name:
            ...
            self._restore_everything_but_user_tables(backup_sub_folder)
            self._restore_user_tables_from_batch_back_ups(backup_sub_folder)
    ...
    def _get_postgres_tool_args(self) -> List[str]:
        ...
        return params + self.additional_pg_tool_args
    ...
    def _backup_everything_but_user_tables(self, temporary_directory_name: str):
    ...
    def _backup_user_tables_in_batches(self, batch_size: int, output_directory: str):
        """
        ...
        sorted_user_table_names = _get_sorted_user_tables_names(
    ...
    def _restore_everything_but_user_tables(self, extracted_backup_location: Path):
    ...
    def _restore_user_tables_from_batch_back_ups(self, extracted_backup_location: Path):
    ...
    def _open_files_and_run_backup(self, backup_file_name: str, batch_size: int):
        with tarfile.open(backup_file_name, "w:gz") as backup_output_tar:
            with tempfile.TemporaryDirectory() as temporary_directory_name:
                self._backup_everything_but_user_tables(temporary_directory_name)
                self._backup_user_tables_in_batches(
                    batch_size, temporary_directory_name
                )
                backup_internal_folder_name = Path(backup_file_name).name
...
def _get_sorted_user_tables_names(connection) -> List[str]:
    """
