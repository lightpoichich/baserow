...
export const state = () => ({
  loading: false,
  loadingRows: false,
  // The timeline view id that is being displayed
  lastTimelineId: null,
  ...
  // based on the date field with startDateFieldId
  dateStacks: {},
  ...
  // is today, the timeline view will be showing the
...
export const mutations = {
  RESET(state) {
    ...
    state.lastTimelineId = null
    state.startDateFieldId = null
    state.endDateFieldId = null
    state.fieldOptions = {}
    state.dateStacks = {}
  ...
  SET_LAST_CALENDAR_ID(state, timelineId) {
    state.lastTimelineId = timelineId
  },
  SET_START_DATE_FIELD_ID(state, startDateFieldId) {
...
export const actions = {
  ...
  async resetAndFetchInitial(
    ...
    { timelineId, startDateFieldId, fields, includeFieldOptions = true }
  ) {
    commit('RESET')
  ...
  async refreshAndFetchInitial(
    ...
    commit('SET_START_DATE_FIELD_ID', startDateFieldId)
    commit('SET_LAST_CALENDAR_ID', timelineId)
  ...
  async fetchMonthly(
    ...
    try {
      const { data } = await TimelineService(this.$client).fetchRows({
        timelineId: getters.getLastTimelineId,
  ...
  async updateAllFieldOptions(
    ...
    const timelineId = getters.getLastTimelineId
    if (!readOnly) {
  ...
  async createdNewRow(
    ...
    const startDateFieldId = getters.getStartDateFieldIdIfNotTrashed(fields)
    const value = row[`field_${startDateFieldId}`]
    const timezone = getters.getTimeZone(fields)
    ...
    const sortings = [{ field: startDateFieldId, value: 'ASC', order: 'ASC' }]
  ...
  async updatedExistingRow(
    ...
    const startDateFieldId = getters.getStartDateFieldIdIfNotTrashed(fields)
    const fieldName = `field_${startDateFieldId}`
  ...
  async selectRow({ commit, getters, dispatch }, { row, fields }) {
    ...
    if (startDateFieldId) {
      ...
      // `timeline/ABC/row/XYZ` URL or by the user clicking on a row they can see
      // in the timeline view. When triggered by a navigation we almost certainly don't
...
export const getters = {
  ...
  getLastTimelineId(state) {
    return state.lastTimelineId
  },
  getStartDateFieldIdIfNotTrashed: (state, getters) => (fields) => {
  ...
  getDateField: (state) => (fields) => {
    const fieldId = state.startDateFieldId
    if (fieldId) {
  ...
  getTimelineTimezoneAndDateLoaded: (state, getters) => (fields) => {
